...
"""Deterministic stage-wise carbon calculation.

**Carried over, not rewritten.** This is 1,300 lines of arithmetic over CSV emission
factors, already covered by a golden oracle
(`tests/reference/carbon/representative_iphone15.json` → 64.852055 kg CO2e). Rewriting
verified arithmetic to make it look like the rest of the codebase would risk the numbers
for a stylistic gain, which is the wrong trade. What changed is the boundary, not the
physics:

- the module-level service singleton is gone — it made two data roots in one process
  impossible, which is the same defect removed from the symbolic layer
- the data root is an explicit constructor argument with no repo-relative default
- `ImpactEngine` conformance lives next door in ``adapter.py`` rather than here

Known behaviour, frozen deliberately: ``calculate(scenario=...)`` currently has **no
effect** — the oracle's grid-scenario capture is byte-identical to its base case. That is
recorded rather than silently fixed, because a port is the wrong moment to change results.
"""
...
# No repo-relative default: the data root is supplied by the composition root.
...
def _transport_mode_alias(name: str | None) -> str | None:
    mapping = {
        "truck": "truck",
        "road": "truck",
        "road_freight": "truck",
...
class RecyclabilityResult:
    ...
    recoverable_mass_kg: float | None
    incineration_mass_kg: float | None
    landfill_mass_kg: float | None
    status: str
    notes: list[str] = field(default_factory=list)
...
class CarbonCalculationResult:
    ...
    uncertainty_pct: float | None = None
    uncertainty_kg_co2e: float | None = None
    uncertainty_range_kg_co2e: dict[str, float] | None = None
...
class CarbonCalculationService:
    ...
    def calculate(
        self, product_id: str, scenario: dict[str, Any] | None = None
    ) -> CarbonCalculationResult:
        scenario = scenario or {}
        product = self.load_product_profile(product_id)
        requested_stages = [
            _stage_alias(stage) for stage in (scenario.get("requested_stages") or DEFAULT_STAGES)
        ]
        ...
        quality_status, estimated_fields, uncertainty_pct, uncertainty_kg, uncertainty_range = (
            self._summarize_quality(
                product=product,
        ...
        provenance_source_refs = _unique(
            ref for item in provenance for ref in item.source_refs if ref
        )
        ...
        return CarbonCalculationResult(
            ...
            source_refs=_unique(
                source_refs
                + self._resolve_source_refs(product, stage_source_refs)
    ...
    def _load_generic_factor_table(
        self, path: Path, value_column: str
    ...
    def _source_ref_map(product: dict[str, Any]) -> dict[str, dict[str, Any]]:
        ...
        return {ref.get("id"): ref for ref in refs if isinstance(ref, dict) and ref.get("id")}
    ...
    @staticmethod
    def _observed_fact(product: dict[str, Any], key: str) -> dict[str, Any] | None:
        facts = product.get("observed_facts", {}) or {}
        value = facts.get(key)
        return value if isinstance(value, dict) else None

    @staticmethod
    def _fact_numeric_value(fact: dict[str, Any] | None) -> float | None:
        if not isinstance(fact, dict):
            return None
        return _to_float(fact.get("value")) or _to_float(fact.get("preferred_value"))
    ...
    @staticmethod
    def _status_looks_estimated(status: str | None) -> bool:
    ...
    def _resolve_generic_factor(
        ...
        assumption_prefix: str | None = None,
    ...
    def _resolve_electricity_factor(
        ...
        if not rows:
            return FactorResolution(
                ...
                status="missing",
                notes=[
                    f"No normalized electricity factor exists for country {country_code.upper()}."
                ],
            )
        ...
        if report_year is not None:
            ...
            chosen = rows[-1]
            notes.append(
                f"No report year provided; used latest available electricity factor year {chosen['year']}."
            )
        ...
        return [
            {
                ...
                "status": "estimated",
                "source_ref_ids": [
                    product.get("bootstrap_estimates", {}).get(
                        "based_on_source_ref_id", "bootstrap"
                    )
                ],
            }
    ...
    def _material_inputs(
        ...
        if use_bootstrap:
            ...
            if estimated_entries:
                assumptions.append(
                    "Used estimated raw-material profile because official material masses are missing."
                )
    ...
    def _calculate_raw_materials_stage(
        ...
        entries = self._material_inputs(
            product, scenario, total_mass_kg, use_bootstrap, assumptions
        )
            ...
            traces.append(
                CarbonTraceItem(
                    ...
                    source_refs=_collect_source_refs(
                        entry.get("source_ref_ids"), entry.get("source_ref"), factor.source_ref
                    ),
                    notes=notes,
                )
            )
        ...
        stage_status = (
            "complete" if traces and not missing_inputs else ("partial" if traces else "missing")
        )
        estimated_inputs = [
        ...
        return CarbonStageResult(
            ...
            missing_inputs=_unique(missing_inputs),
            quality_status=quality_status,
            uncertainty_pct=self._stage_uncertainty_default(
                product, "raw_materials", quality_status
            ),
    ...
    def _transport_inputs(
        ...
        if estimated_entries:
            assumptions.append(
                "Used estimated transport legs because official transport-route data is missing."
            )
            return estimated_entries
        return official_entries
    ...
    def _calculate_transport_stage(
        ...
        return CarbonStageResult(
            ...
            quality_status=quality_status,
            uncertainty_pct=self._stage_uncertainty_default(
                product, "transportation", quality_status
            ),
            estimated_inputs=_unique(estimated_inputs),
        )
    ...
    def _calculate_use_phase_stage(
        ...
        if use_bootstrap:
            ...
            if estimated_use:
                ...
                if use_data != before:
                    ...
                    assumptions.append(
                        "Used estimated use-phase defaults where official MX431adn use data is missing."
                    )
        ...
        include_paper = bool(
            use_data.get(
                "include_paper_default", product.get("defaults", {}).get("include_paper", False)
            )
        )
        ...
        missing_inputs: list[str] = []
        if (
            lifetime_energy_kwh is None
            and annual_energy_kwh is not None
            and lifetime_years is not None
        ):
            lifetime_energy_kwh = annual_energy_kwh * lifetime_years
            assumptions.append(
                "Derived lifetime use-phase electricity from annual energy and lifetime years."
            )
        if lifetime_energy_kwh is None:
            missing_inputs.append("use_phase:lifetime_energy_kwh is missing.")
            notes.append(
                "Provide lifetime_energy_kwh or both annual_energy_kwh and lifetime_years."
            )

        factor = self._resolve_electricity_factor(
            country_code=country_code,
            report_year=report_year,
            factor_key=use_data.get("electricity_factor_key")
            or product.get("defaults", {}).get("electricity_factor_key"),
        ...
        if include_paper:
            warnings.append(
                "Paper-inclusive use-phase calculation is not implemented yet; current calculation excludes paper impacts."
            )
        ...
        trace = CarbonTraceItem(
            ...
            status=status,
            source_refs=_collect_source_refs(
                use_data.get("source_ref_ids"), use_data.get("source_ref"), factor.source_ref
            ),
            notes=notes,
        )
    ...
    def _calculate_end_of_life_stage(
        ...
        if use_bootstrap:
            ...
            if estimated_eol:
                ...
                if eol_data != before:
                    used_estimated_defaults = True
                    assumptions.append(
                        "Used estimated end-of-life defaults where the declared MX431adn scenario is missing."
                    )
        ...
        routes = [
            (
                "recycling",
                _to_float(eol_data.get("recycling_rate_pct"))
                or _to_float(eol_data.get("recyclability_pct")),
            ),
            (
                "incineration",
                _to_float(eol_data.get("incineration_rate_pct"))
                or _to_float(eol_data.get("incineration_pct")),
            ),
            (
                "landfill",
                _to_float(eol_data.get("landfill_rate_pct"))
                or _to_float(eol_data.get("landfill_pct")),
            ),
        ]
        ...
        if not provided_rates:
            ...
            warnings.append(
                f"End-of-life route percentages sum to {sum(provided_rates):.3f} instead of 100."
            )
        ...
        for route, rate_pct in routes:
            ...
            traces.append(
                CarbonTraceItem(
                    ...
                    status=status,
                    source_refs=_collect_source_refs(
                        eol_data.get("source_ref_ids"),
                        route_factor.get("source_ref"),
                        factor.source_ref,
                    ),
                    notes=notes,
                )
    ...
    def _calculate_recyclability(
        ...
        if use_bootstrap:
            ...
            if estimated_eol:
                eol_data = self._merge_missing_values(eol_data, estimated_eol)
        recycling_rate = _to_float(eol_data.get("recycling_rate_pct")) or _to_float(
            eol_data.get("recyclability_pct")
        )
        incineration_rate = _to_float(eol_data.get("incineration_rate_pct")) or _to_float(
            eol_data.get("incineration_pct")
        )
        landfill_rate = _to_float(eol_data.get("landfill_rate_pct")) or _to_float(
            eol_data.get("landfill_pct")
        )
        ...
        if (
            recycling_rate is not None
            and incineration_rate is not None
            and landfill_rate is not None
            ...
            entries.append(
                CarbonProvenanceItem(
                    ...
                    source_refs=self._resolve_source_refs(
                        product, observed_mass.get("source_ref_ids", [])
                    ),
            ...
            if fact and value is not None:
                entries.append(
                    CarbonProvenanceItem(
                        ...
                        method="official_observation"
                        if fact.get("status") == "exact"
                        else "derived_from_observation",
                        source_refs=self._resolve_source_refs(
                            product, fact.get("source_ref_ids", [])
                        ),
        ...
        if tec_fact and ("use_phase" in result or used_bootstrap):
            entries.append(
                CarbonProvenanceItem(
                    ...
                    source_refs=self._resolve_source_refs(
                        product, tec_fact.get("source_ref_ids", [])
                    ),
                    notes=[str(tec_fact.get("selection_reason") or "")]
                    + list(tec_fact.get("notes", [])),
                    uncertainty_pct=10.0,
                )
            )
        ...
        if result.get("raw_materials") and result["raw_materials"].quality_status == "estimated":
            ...
            entries.append(
                CarbonProvenanceItem(
                    ...
                    source_refs=self._resolve_source_refs(
                        product, ["sample_mx622_profile", "internal_estimation_profile"]
                    ),
                    notes=[
                        "Derived from the nearest structured Lexmark family reference because official BOM masses are missing."
                    ],
                    uncertainty_pct=self._stage_uncertainty_default(
                        product, "raw_materials", "estimated"
                    ),
                )
            )
        ...
        if result.get("transportation") and result["transportation"].quality_status == "estimated":
            ...
            entries.append(
                CarbonProvenanceItem(
                    ...
                    notes=[
                        "Used generic ship plus truck distribution defaults because the real route is not declared."
                    ],
                    uncertainty_pct=self._stage_uncertainty_default(
                        product, "transportation", "estimated"
                    ),
                )
            )
            ...
            entries.append(
                CarbonProvenanceItem(
                    ...
                    value={
                        ...
                        "electricity_factor_value_kg_co2e_per_kwh": use_phase.get(
                            "electricity_factor_value_kg_co2e_per_kwh"
                        ),
                    ...
                    source_refs=self._resolve_source_refs(
                        product, use_phase.get("source_ref_ids", [])
                    ),
                    notes=list(use_phase.get("notes", [])),
                    uncertainty_pct=self._stage_uncertainty_default(
                        product, "use_phase", "estimated"
                    ),
                )
            )
    ...
    def _summarize_quality(
        ...
        total_kg_co2e: float | None,
        partial_total_kg_co2e: float,
    ) -> tuple[str, list[str], float | None, float | None, dict[str, float] | None]:
        ...
        weighted_base = 0.0
        total_reference = (
            total_kg_co2e if total_kg_co2e is not None else (partial_total_kg_co2e or 0.0)
        )
        ...
        for stage_name, stage in stage_results.items():
            if stage.quality_status == "estimated":
                ...
                has_override = True
            stage_uncertainty = stage.uncertainty_pct or self._stage_uncertainty_default(
                product, stage_name, stage.quality_status
            )
        ...
        if has_estimated:
            ...
            quality_status = (
                "scenario_override" if total_kg_co2e is not None else "partial_override"
            )
        else:
        ...
        if total_reference and total_reference > 0:
            if weighted_base > 0:
                ...
                uncertainty_pct = _to_float(
                    product.get("estimation_profile", {})
                    .get("uncertainty_defaults_pct", {})
                    .get("total")
                )
...
# The module-level ``calculate_carbon_footprint`` helper went with the singleton it
# called. Callers construct a ``CarbonCalculationService`` with an explicit data
# root, or take ``CsvFactorImpactEngine`` from the bundle.
